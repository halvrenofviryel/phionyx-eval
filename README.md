...
The Phionyx **Reasoned Governance Envelope (RGE)** is developed alongside AIREP — a conformant projection between the two is **not released** — an experimental Decision-only projection is implemented, has not been independently rerun, and Control, Execution and Effect remain NOT_PROJECTABLE (the 2026-08-06 measurement that AIREP's own reference verifier rejects an RGE envelope handed to it directly is why a projection layer is required) — maturing by conforming to the format. AIREP is an experimental, proposed open format and not a ratified standard. External implementation evidence is scoped per version — an independently authored producer measured against frozen v0.1.2, and an independently implemented consumer/verifier measured against v0.2 — which target different frozen versions and do not establish producer↔consumer interoperability.
