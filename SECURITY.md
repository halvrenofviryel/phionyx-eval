...
`phionyx-eval` is an LLM-as-judge evaluation primitive with deterministic verdict derivation. It is **alpha / experimental** software. Its value depends on its
...
**founder@phionyx.ai** with `[SECURITY] phionyx-eval` in the subject. Please do not open a public
issue or PR for a suspected vulnerability until it has been triaged.

Include, where you can: the version (`pip show phionyx-eval`), a minimal reproduction, and what
